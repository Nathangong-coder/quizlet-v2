...
describe('kltRowsToTopicRows', () => {
  it('splits live from superseded KLPs, as category rows do', () => {
    const [row] = kltRowsToTopicRows(
      [{
        normalizedName: 'wacc', name: 'WACC', depth: 2, parentName: null,
  ...
  it('includes every rank at the default maxRank', () => {
    const [row] = kltRowsToTopicRows(
      [{ normalizedName: 'wacc', name: 'WACC', depth: 2, parentName: null, links: [link('a', 1), link('b', 3)] }],
      3,
    )
  ...
  it('drops ranks above maxRank when the knob is narrowed', () => {
    const [row] = kltRowsToTopicRows(
      [{ normalizedName: 'wacc', name: 'WACC', depth: 2, parentName: null, links: [link('a', 1), link('b', 2)] }],
      1,
    )
    expect(row.klpIds).toEqual(['a'])
  })
  ...
  it('carries cardIds so whole-answer tags can still be attributed', () => {
    ...
    const [row] = kltRowsToTopicRows(
      [{ normalizedName: 'wacc', name: 'WACC', depth: 2, parentName: null, links: [link('a', 1)] }],
      3,
    )
  ...
  it('never emits a topic whose links are all superseded', () => {
    expect(
      kltRowsToTopicRows(
        [{ normalizedName: 'dead', name: 'Dead', depth: 2, parentName: null, links: [link('a', 1, new Date())] }],
        3,
      ),
    ).toEqual([])
  })
  ...
  it('dedupes a KLP linked twice at different ranks', () => {
    const [row] = kltRowsToTopicRows(
      [{ normalizedName: 'wacc', name: 'WACC', depth: 2, parentName: null, links: [link('a', 1), link('a', 2)] }],
      3,
    )
    expect(row.klpIds).toEqual(['a'])
  })
...
describe('kltRowsToTopicRows — subtree rollup', () => {
  it('counts a descendant’s key points toward an ancestor', () => {
    ...
    const [row] = kltRowsToTopicRows(
      [{
        normalizedName: 'accounting', name: 'accounting', depth: 1, parentName: null,
  ...
  it('carries depth through so the display can group by level', () => {
    const [row] = kltRowsToTopicRows(
      [{ normalizedName: 'accounting', name: 'accounting', depth: 1, parentName: null, links: [link('a', 1)] }],
      3,
    )
