import { describe, it, expect } from 'vitest'
import { existsSync, readFileSync } from 'node:fs'
import { join } from 'node:path'

/**
 * Match is a GAME and writes nothing (owner's decision, 2026-09-13; it had
 * recorded a StudySession + StudyEvents since Stage 1). A source scan is the
 * guard: the moment the component or the page imports a server action, or
 * the old action file comes back, this fails.
 */
const FILES = ['src/components/game/MatchGame.tsx', 'src/app/sets/[id]/match/page.tsx']

describe('Match writes nothing', () => {
  it.each(FILES)('%s imports no server action and no memory writer', (file) => {
    const src = readFileSync(join(process.cwd(), file), 'utf8')
    expect(src).not.toMatch(/from ['"]@\/actions\//)
    expect(src).not.toMatch(/recordStudyEvent|startStudySession|submitMatchSession|finishStudySession/)
  })

  it('the old match-session action is gone', () => {
    expect(existsSync(join(process.cwd(), 'src/actions/match-session.ts'))).toBe(false)
  })

  it('/match is no longer behind the sign-in middleware — nothing to protect', () => {
    const mw = readFileSync(join(process.cwd(), 'src/middleware.ts'), 'utf8')
    expect(mw).not.toContain('/match')
  })
})
