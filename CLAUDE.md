...
1. **Production:** each user stores their own provider credentials in `AiCredential` — many per user, not one. Fields: `provider` (`google | anthropic | openai | openrouter | deepseek | zai | custom`), `label`, `defaultModel`, `role` (`primary | backup`), `enabled`, `lastUsedAt`, `lastErrorKind`. An optional `AiTaskRouting` row pins a task (`grade | plan | distractors | autocomplete` — see `AI_TASKS` in `src/lib/ai/model-routing.ts`) to one specific credential + model; without a pin, every enabled credential is eligible, ordered by the task's **measured default** (`src/lib/ai/task-defaults.ts`, from `npm run bench-models` 2026-09-15: judgment tasks prefer DeepSeek, writing tasks — distractors, game pieces, Hot Seat, authoring — prefer Z.ai GLM; own keys before borrowed, LRU within a provider). Z.ai is first-class: the schema rides in the prompt and `reasoning_effort` is `low` for every runtime task, `high` for authoring — the default effort takes 30-40 s a call. A routing **model override is only valid alongside a pinned credential** (`saveTaskRouting` rejects a model with no credential, and `generateJson` ignores one) — a model id is provider-specific, so applying it across a heterogeneous pool 404s every non-matching provider and false-badges healthy keys. Credential CRUD, model listing, and credential testing live in `src/actions/ai-credentials.ts`.
...
**KLP authoring is discrimination-tested, not merely generated (Stage 8 Spec 2, `npm run author-klps`).** For one card at a time, `src/lib/klp/authoring.ts` drafts a reference answer and a KLP set, has a grader-family model review the reference (accuracy / conciseness / clarity — the **communication check**, `review-reference.ts`; the writer rewrites once when TypeScript says the labels warrant it), then writes two deliberately wrong candidate answers (`vague`, `memorized_template` — `PROBE_KINDS`; `confident_wrong` was cut 2026-09-13 because it never set the best-wrong bar and lives on only as a legacy kind on stored rows), grades every candidate against the current KLPs and computes a **separation score in TypeScript** (`src/lib/klp/separation.ts`) from the AI's per-candidate categorical verdicts — the score is never an AI opinion. A definition/contrast point the template trap recites is a **framing** point (`src/lib/klp/framing.ts`, `CardKlp.role`): kept and mapped to topics, but excluded from the **substance separation** the bar reads. Below `SEPARATION_FLOOR` (`src/lib/klp/authoring-config.ts`), the KLPs are revised and re-graded up to `MAX_REVISIONS` times — **one combined revise call per round** carrying the separation findings, the **rebuild test's** parity losses and coverage misses (`rebuild.ts`: an answer rebuilt from the points alone, graded against the card's own points and the reference; `REBUILD_PARITY_BAR` 0.7), and the **compression** findings (`compression.ts`: the grader reviews the rebuilt answer against the numbered points and names restated / bloated / off-card points; a `verbose` hygiene rule; the rebuilt/reference word ratio) under one precedence rule, *cut words, never distinct claims*. Regrading is **incremental** (`regrade-plan.ts`): verdicts on unchanged points carry by exact text, only rewritten points are graded, and only the trap that beat the set is rewritten; a card that still won't separate is persisted anyway and flagged `low_discrimination` (`CardAuthoring.status`) rather than silently retried until it looks clean — that flag is the entire point of the loop. **Each grading call is isolated per candidate** (`GRADE_CANDIDATES_SEPARATELY`, on by default): the grader is shown one candidate at a time, never all four together, because a grader that can see every candidate ranks them against each other and manufactures separation the KLPs never earned. **Weight is computed from two signals, never an AI centrality rating** (`weightFromSignals`, `src/lib/klp/relations.ts`) — a **blast-radius** term (how many other KLPs on the card break if this one is false, off `KlpRelation` edges that survive cycle/out-of-range pruning) blended with a **discrimination-breadth** term (how many of the adversaries fail this KLP, read off call B's verdict matrix at no extra AI cost). Both terms exist because neither works on every card shape: blast radius measures dependency depth, so it spans its range on a *derivation chain* and collapses flat on an *enumeration* — and the fix for a flat enumeration is never to push the relate prompt for more edges, since a fabricated `causes` edge makes Spec 3 serve a probe for a link that does not exist. The legacy single-pass extractor (`src/lib/ai/prompts/extract-klps.ts`) still authors newly created cards until a later spec migrates card creation onto this pipeline; `author-klps` is an operator tool run per-set (`--set <setId>`, required; it never walks the whole corpus), with `--direct` bypassing the stored-credential pool to use a raw `GOOGLE_API_KEY` for local runs. **`--direct` rotates over the cross product of `GOOGLE_API_KEYS` x `KLP_DIRECT_MODELS`** (`src/lib/klp/direct-pool.ts`), because the Gemini free-tier cap is **20 requests per day, per model, per project** — roughly three authored cards per model per day. It reuses `selectAttemptOrder` rather than growing a second LRU, and **pins one combo per card**: a separation score subtracts candidate scores, so grading a card's four candidates on different models would fold the gap between two graders into the number meant to measure the gap between a strong and a weak answer. A per-day quota retires that combo and the run continues; a malformed structured reply is retried once in place, and any other non-quota failure moves the card to the next model (models differ materially in structured-output compliance). The production configuration since 2026-09-12 is a **role split**: `KLP_AUTHOR_PROVIDER=zai` / `glm-5.3-flash` writes (and rewrites), `KLP_DIRECT_PROVIDER=deepseek` reviews, writes the traps, rebuilds and grades (`KLP_REVISE_WITH=grader`, `KLP_ADVERSARIES_WITH=grader`, `KLP_GRADE_STRICT=true` — strict grading sees each point's kind and relaxes on mechanism/condition/quantitative). Every run prints a per-step token and list-price table (`src/lib/klp/token-meter.ts`); a card costs about half a cent off-peak. `docs/ai/model-performance.md` is the operator record of every measurement behind these choices.

**The weight histogram is the acceptance criterion for that formula, not a report on it (`npm run klp-histogram`).** Audit finding G1 was a measurement, so only a measurement can close it. **Measured baseline, whole live corpus, 2026-09-04: 272 live KLPs, all legacy-extracted — 92.3% at weight 4-5, mean 4.54, and 62.5% at the single value 5.** **G1 was then closed on the same measurement (2026-09-04): the 10-card LBO pilot's 50 authored KLPs read 22.0% at 4-5, 40.0% at 1-2, mean 2.86, verdict OK.** Findings are suppressed below `HISTOGRAM_MIN_SAMPLE` (20) — a card carries only 4-9 KLPs, and a check that fires on every single-card run gets ignored. `scripts/klp-histogram.ts` is read-only, splits the corpus into **authored vs legacy by `(cardId, klpVersion)`** (a mixed number answers nothing), and names three failure modes (`src/lib/klp/histogram.ts`), each with a different cause: `clustered_high` (≥75% at 4-5 — G1 itself; something is inflating), `clustered_low` (≥75% at 1-2 — both terms flat, so check the **discrimination-breadth histogram** printed beside it before touching the relate prompt), and `uniform` (one weight value ≥60% of the corpus — flatness in the middle, invisible to both tail checks). It reports **every** mode that fires, not one verdict. `npm run author-klps` prints the same histogram over its own run at the end, so a run that posts a healthy mean separation and a useless weight signal is visible without a second command — those two numbers measure different things.
