...
- **DECIDED 2026-08-08, not yet built.** "Forget" must clear the data it stored *and* everything it fed, and reset gains a granular per-quiz / per-question form. Design and build tracked as queue item 2 in `docs/superpowers/BUILD-QUEUE.md`; delete the paragraph below once it ships. Original framing, still useful as the design's starting point:
- **What "forget" should do to the knowledge posterior was undecided.** `resetUserMemory` (`src/actions/user.ts`) clears `KlpState` as of the 2026-08-08 hardening pass (defect B3) — it deletes `QuizAttempt`/`QuizAnswer`, so the `AnswerKlpResult` evidence goes with them, and a posterior left standing would claim knowledge from rows that no longer exist and be beyond the backfill's reach forever. The **selective** resets, `forgetCard` and `forgetSet` (`src/actions/memory.ts`), do NOT clear it — and that is not the same bug: neither deletes `QuizAnswer`, so the evidence survives and the posterior stays consistent with it (a replay would recompute the same numbers). But "forget this card" leaving its KLP knowledge intact is the same *surprise* B3 fixed at the account level. Two coupled questions need an explicit product decision: does "forget" mean **drop the estimate** or **drop the evidence too**, and should the two verbs even agree? Deliberately left alone during hardening rather than answered silently inside a bug-fix pass.
