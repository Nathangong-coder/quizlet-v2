/**
 * The KLT summarization pipeline.
 *
 * A PLAIN MODULE, deliberately not a `'use server'` action. Two reasons:
 *
 * 1. `summarizeKltsForCards` takes a `userId` as its first argument. Exported
 *    from an action file it would be a client-callable RPC endpoint that
 *    accepts an arbitrary user id — owner-scoped internally by
 *    `readableSetWhere`, but it has no business being reachable at all.
 * 2. An action module pulls `auth` and `next/cache` in with it, so
 *    `scripts/backfill-klts.ts` could not import it without dragging the whole
 *    Next client runtime into a plain node process.
 *
 * `src/actions/klt.ts` keeps the thin retry action that genuinely needs a
 * session and a revalidate.
 */
