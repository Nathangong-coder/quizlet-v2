...
export default async function FolderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="w-full space-y-8">
      ...
      <header className="flex items-start gap-4 border-b border-border/70 pb-7"><div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-slate-600 dark:bg-slate-900/70 dark:text-slate-300 sm:h-20 sm:w-20"><Folder className="h-8 w-8 sm:h-10 sm:w-10" aria-hidden="true" /></div><div className="min-w-0 space-y-2"><div className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">Your folder</div><h1 className="text-3xl font-semibold tracking-tight sm:text-5xl">{folder.name}</h1><p className="max-w-3xl text-base leading-relaxed text-muted-foreground">{folder.description || 'Keep the study material and lived experience around one goal in the same place.'}</p></div></header>
