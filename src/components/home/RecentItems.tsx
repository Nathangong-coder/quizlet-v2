...
export function RecentItems({ items }: { items: HomeRecentItem[] }) {
  ...
  return <ul className="grid gap-x-10 sm:grid-cols-2">{items.map((item) => {
    ...
    return <li key={`${item.kind}-${item.id}`}><Link href={item.href} className="group flex min-w-0 items-center gap-3 border-b border-border/70 py-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:py-4"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${COLORS[item.kind]}`}><Icon className="h-5 w-5" aria-hidden="true" /></div><div className="min-w-0"><h3 className="truncate text-sm font-semibold tracking-tight group-hover:text-primary sm:text-base">{item.title}</h3><p className="mt-0.5 truncate text-xs text-muted-foreground sm:text-sm">{item.kindLabel}{item.meta && <> <span aria-hidden="true">·</span> {item.meta}</>} <span aria-hidden="true">·</span> {item.byline}</p></div></Link></li>
  })}</ul>
}
