 ...
 * sync for free.
 */
export function MobileRail({ signedIn, role, folders, groups = [], unread = 0 }: { signedIn: boolean; role?: string | null; folders: RailFolder[]; groups?: RailGroup[]; unread?: number }) {
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative flex h-full w-72 flex-col border-r border-sidebar-border bg-sidebar p-5">
            ...
            <div className="flex-1 overflow-y-auto">
              <RailNav signedIn={signedIn} role={role} folders={folders} groups={groups} unread={unread} onNavigate={() => setOpen(false)} />
            </div>
          </div>
        </div>
      )}
