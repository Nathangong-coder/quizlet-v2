...
- [ ] **Step 1: Failing tests.** Study is not current on `/knowledge` or `/analysis`; at most
...
- [ ] **Step 3: Implement** per spec §3 and §5.3.
- [ ] **Step 4: Mutate** `knowledge ?? 0` and `startsWith`; confirm both red; confirm each
...
- [ ] **Step 1: Failing tests.** A null `knowledge` survives as null through
...
- [ ] **Step 2: Run, verify fail.**
- [ ] **Step 3: Implement.** `loadSetKnowledge` is a thin read-only shell over
...
- [ ] **Step 1:** Move the Study page into `(views)` with `git mv`; fall back to copy+remove
      if OneDrive refuses the rename.
- [ ] **Step 2:** Lift the header out of the page and into the layout — title, description,
      fork attribution, card count, owner actions, listing-blocked notice. Drop the outline
      **Concepts** button; it is a tab now.
- [ ] **Step 3:** `SetViewTabs` is the only client piece, using `isSetViewCurrent`.
- [ ] **Step 4:** Update `ENFORCED_PATHS` and the route-structure lists for the new paths, and
      assert `edit` and `concepts` are NOT inside `(views)`.
- [ ] **Step 5: Mutate** by moving `edit` into `(views)`; confirm red.
...
- [ ] **Step 1:** Four-metric header via `Metric`, which already renders `—` for null.
- [ ] **Step 2:** `ConceptCanvas` gains an optional `shades?: Map<string, MasteryShade>`.
      Optional so `/sets/[id]/concepts` renders exactly as it does now — the editor is not
      changing.
- [ ] **Step 3:** `MasteryList` takes `TopicMasteryRow[]` and imports nothing from KLT. The
      view is chosen by `?view=list`.
- [ ] **Step 4:** Categories, confidence distribution, session history.
- [ ] **Step 5:** Signed-out branch: unshaded map plus a sign-in prompt, never a 404.
...
- [ ] **Step 1:** Render `RetentionPanel`, misconceptions and pace outliers from the same
      scoped metrics.
- [ ] **Step 2:** `InProgressBlock` states what is coming and what it needs. **No fake chart** —
      a dimmed invented chart is indistinguishable from real data at a glance.
- [ ] **Step 3:** Empty states route through `diagnoseEmptyState` so four causes stay four.
...
- [ ] **Step 2:** Re-run every mutation from Tasks 1 and 3 against the final tree.
- [ ] **Step 3:** Update `docs/superpowers/BUILD-QUEUE.md` with baselines and the owed gate.
- [ ] **Step 4:** Update memory: `set-views-and-atlas-owed` becomes built; record the three
      revised decisions.
- [ ] **Step 5: Commit.**
