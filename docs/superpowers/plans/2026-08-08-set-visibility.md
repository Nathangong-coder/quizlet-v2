...
- Signed-out access to a link-shared set is a **requirement**, not an oversight. An early `if (!session) notFound()` on the set detail or match page would be a regression. `/quiz`, `/review` and `/print` keep their existing sign-in requirement — those need an account regardless.
